...
void *forward_messages(void *arg) {
    ...
    log("INFO", "[forward] forward_messages: Started forwarding messages (thread index %d)\n", info->live_thread_index);
    ...
    while ((nbytes = sctp_recvmsg(info->source_socket, buffer, sizeof(buffer), NULL, 0, NULL, NULL)) > 0) {
        ...
        if (dest_sock > 0) {
            ...
            if (sent < 0) {
                log_perror("[forward] forward_messages: sctp_sendmsg error");
            }
        }
        log("INFO", "[forward] forward_messages: Transfered %d bytes of data; sockets %d->%d\n",nbytes, info->source_socket, dest_sock);
...
void *handle_gnb_connection(void *arg) {
    ...
    log("INFO", "[forward] handle_gnb_connection: Handling new gNB socket %d\n", gnb_socket);
    ...
    total_conn_count++;
    log("INFO", "[forward] handle_gnb_connection: Connected to AMF id=%d, incremented connections to %d; total_conn_count=%d\n",
